...
# Constants begin.
UPSTREAM_REPO_URL = "https://github.com/Dark-Princ3/X-tra-Telegram"


@borg.on(admin_cmd("updhate ?(.*)"))
async def upstream(ups):
    ...
    if conf != "now" and not force_update:
        changelog_str = f'**New UPDATE available for X-tra-Telegram\n\nCHANGELOG:**\n`{changelog}`'
    ...
    if Var.HEROKU_API_KEY is not None:
        ...
        if not Var.HEROKU_APP_NAME:
            await ups.edit(
                '`Please set up the Var.HEROKU_APP_NAME variable to be able to update userbot.`'
            )
            repo.__del__()
            return
        ...
        await ups.edit(f"`[Updater]\
                        Your userbot's dyno build in progress, please wait for it to complete.\n It may take 10 minutes `"
                       )
        ups_rem.fetch(ac_br)
